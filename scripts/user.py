    ...
    if r.status_code == 200:
        ...
        if len(j) > 0:
            if username != "":
                ...
                for u in j:
                    print(f"{u['id']}: Name:{u['username']} Home:{u['home_dir']}")
    return None
...
def adduser(uname="gg", upass="ll", home="/", pubkey="", 
        perms=['list', 'download', 'upload', 'delete', 'rename', 'create_dirs'],
        ):
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        cmd = sys.argv[1]
        if cmd == 'add':
            #adduser(uname=sys.argv[2], upass=sys.argv[3], home=sys.argv[4], perms=['*',])
            adduser(uname=sys.argv[2], upass=sys.argv[3], home=sys.argv[4], perms=['*','_expire:5'])
        elif cmd == 'del':
