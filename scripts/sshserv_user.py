...
def adduser(uname="gg", upass="ll", home="/", pubkey="", perms=['list', 'download', 'upload', 'delete', 'rename', 'create_dirs']):
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        cmd = sys.argv[1]
        if cmd == 'add':
            adduser(uname=sys.argv[2], upass=sys.argv[3], home=sys.argv[4])
        elif cmd == 'del':
